...
productsRouter.get('/:pid', async (request, response) => {
 ...
 productsRouter.post('/', async (request, response) => {
  ...
  productsRouter.delete('/:pid', async (request, response) => {
    try {
      const id = request.params.pid;
      // Llama al método deleteProduct del ProductManagerMongo para eliminar el producto por ID
      await ProdMan.deleteProduct(id);
      response.send(`Producto eliminado correctamente`);
    } catch (error) {
      // Maneja cualquier error que pueda ocurrir durante la eliminación
      console.log(error);
      response.status(500).send(`Error al eliminar el producto: ${error.message}`);
    }
  });
